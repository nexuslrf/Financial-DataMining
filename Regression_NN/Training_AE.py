...
def main():
    ...
    net = AE(train_data.shape[1], args.feature_size, args.layers, args.no_BN, nn.__dict__[args.activation])
    ...
    train_log = open(f'train_log_AE_{args.suffix}.log', 'w')
    val_log = open(f'val_log_AE_{args.suffix}.log', 'w')
    ...
    for epoch in range(args.epochs):
        # training
        loss_train = train(train_loader, net, criterion, optimizer)
        train_log.write('{:.6f}\n'.format(loss_train))
        loss_val = validate(val_loader, net, criterion)
        val_log.write('{:.6f}\n'.format(loss_val))
        print(f"Epoch {epoch} Train Loss Avg {loss_train} Eval Loss Avg {loss_val}")

        sched.step()

        save_dir = 'checkpoint_AE_{}.pth.tar'.format(args.suffix)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, 'model_best_AE_{}.pth.tar'.format(args.suffix))
